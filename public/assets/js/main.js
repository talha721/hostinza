...
(function ($) {
  ...
  function skewBgWidthCalc() {
    var skewBg = $(".xs-banner.skew-bg"),
      width = $(window).width();
    skewBg.append(
      "<style>.xs-banner.skew-bg::after{border-left: " +
        width +
  ...
  $(document).ready(function () {
    ...
    if ($(".xs-tweet").length > 0) {
      $(".xs-tweet").twittie({
        dateFormat: "%b. %d, %Y",
        template:
          '{{tweet}} <div class="date">{{date}}</div> <a href="{{url}}" target="_blank">Details</a>',
        count: 2,
      ...
      if (xs_contact_error === false) {
        ...
        $.ajax({
          ...
          success: function success(result) {
            xs_contact_submit
              .after(
                '<p class="xpeedStudio_success_message">' + result + "</p>"
              )
              .hide()
              .fadeIn();
    ...
    if ($(".pricing-matrix-slider").length > 0) {
      ...
      var sliderItems = $(this).find(
        ".pricing-matrix-slider .pricing-matrix-item"
      ).length;
    ...
    if ($(".xs-feature-section").length > 0) {
      ...
      if ($(".xs-footer-section").prev().hasClass("xs-bg-gray")) {
        $(".xs-footer-section")
          .children(".xs-feature-section")
          .addClass("xs-bg-gray");
      } else {
        $(".xs-footer-section")
          .children(".xs-feature-section")
          .removeClass("xs-bg-gray");
      }
    }
  ...
  if ($("#xs-map").length > 0) {
    var locations = [
      ...
      [
        ...
        151.259052,
        5,
        "assets/images/map-marker-1.png",
      ],
      [
        "Cronulla Beach",
        -34.028249,
